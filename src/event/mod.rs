...
impl<'a> Data<'a> {
    ...
    pub(crate) async fn chests(&self) -> wheel::Result<ChestAppearances> {
        ...
        Ok(match (self.series, &*self.event) {
            ...
            (Series::League, "10") => from_file!("../../assets/event/league/chests-10-9.1.28.json"),
            (Series::Mentor, "2026" | "w") => from_file!("../../assets/event/mentor/chests-2026-9.1.json"),
            ...
            (Series::MixedPools, "5") => from_file!("../../assets/event/mp/chests-5-9.0.42-fenhl.2.riir.2.json"),
    ...
    pub(crate) fn draft_kind(&self) -> Option<draft::Kind> {
        match (self.series, &*self.event) {
            (Series::Multiworld, "2") => Some(draft::Kind::MultiworldS2),
    ...
    async fn practice_buttons(&self, global: &GlobalState, csrf: Option<&CsrfToken>, errors: &mut RawHtml<String>, ctx: PracticeButtonsContext) -> Result<PracticeButtons, Error> {
        let practice_seed_urls = if let Some(draft_kind) = self.draft_kind() {
            match draft_kind {
                ...
                draft::Kind::MultiworldS2 | draft::Kind::MultiworldS3 | draft::Kind::MultiworldS4 | draft::Kind::MultiworldS5 | draft::Kind::S7 => vec![
                    (true, uri!(practice_seed_post(self.series, &*self.event, Some(PracticeSeedKind::Base), _)).to_html(), practice_seed_favicon_url(global, self, None).await?, format!("Roll Seed (Base Settings)")),
                    (true, uri!(practice_seed_post(self.series, &*self.event, Some(PracticeSeedKind::Random), _)).to_html(), practice_seed_favicon_url(global, self, None).await?, format!("Roll Seed (Random Settings)")),
                ],
                draft::Kind::RslS7 => vec![
                    (true, uri!(practice_seed_post(self.series, &*self.event, Some(PracticeSeedKind::Base), _)).to_html(), practice_seed_favicon_url(global, self, None).await?, format!("Roll Seed (RSL Weights)")),
                    (true, uri!(practice_seed_post(self.series, &*self.event, Some(PracticeSeedKind::BaseLite), _)).to_html(), practice_seed_favicon_url(global, self, None).await?, format!("Roll Seed (RSL-Lite Weights)")),
                ],
                draft::Kind::SlugOpen => {
                    ...
                    for format in all::<sco::Format>() {
                        if let Some(draft_kind) = format.draft_kind() {
                            ...
                            urls.push((true, uri!(practice_seed_post(self.series, &*self.event, _, Some(format))).to_html(), practice_seed_favicon_url(global, self, Some(format)).await?, format!("Roll Seed ({})", format.display_name())));
                        }
                    }
                    urls
                }
            ...
            vec![(
                ...
                practice_seed_favicon_url(global, self, None).await?,
                format!("Roll Seed"),
            )]
        } else {
            Vec::default()
...
pub(crate) async fn submit_async(global: &GlobalState, me: User, uri: Origin<'_>, csrf: Option<CsrfToken>, series: Series, event: &str, form: Form<Contextual<'_, SubmitAsyncForm>>) -> Result<RedirectOrContent, StatusOrError<Error>> {
    ...
    Ok(if let Some(ref value) = form.value {
        ...
        let times = if let Series::TriforceBlitz = series {
            ...
            vec![time; data.team_config.roles().len()]
        } else {
            vec![
