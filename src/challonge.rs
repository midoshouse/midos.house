use crate::prelude::*;

/// Returns a list of races to import. The `phase`, `round`, and `game` fields will be left blank since the data required to fill them in is not
/// provided by the Challonge API, and only one race for each match is imported. The caller is expected to fill in the values for `phase` and
/// `round`, duplicate this race to get as many different games as there should be in the match, and create a single scheduling thread for the match.
pub(crate) async fn races_to_import(transaction: &mut Transaction<'_, Postgres>, http_client: &reqwest::Client, config: &Config, event: &event::Data<'_>, community: Option<&str>, tournament: &str) -> Result<Vec<Race>, cal::Error> {
    #[derive(Deserialize)]
    struct Matches {
        data: Vec<Match>,
        //TODO pagination (links.next)
    }

    #[derive(Deserialize)]
    struct Match {
        id: String,
        relationships: Relationships,
    }

    #[derive(Deserialize)]
    struct Relationships {
        player1: Player,
        player2: Player,
    }

    #[derive(Deserialize)]
    struct Player {
        data: PlayerData,
    }

    #[derive(Deserialize)]
    struct PlayerData {
        id: String,
    }

    let mut races = Vec::default();
    let Matches { data } = http_client.get(if let Some(community) = community {
        format!("https://api.challonge.com/v2/communities/{community}/tournaments/{tournament}/matches.json")
    } else {
        format!("https://api.challonge.com/v2/tournaments/{tournament}/matches.json")
    })
        .header(reqwest::header::ACCEPT, "application/json")
        .header(reqwest::header::CONTENT_TYPE, "application/vnd.api+json")
        .header("Authorization-Type", "v1")
        .header(reqwest::header::AUTHORIZATION, &config.challonge_api_key)
        .send().await?
        .detailed_error_for_status().await?
        .json_with_text_in_error().await?;
    for set in data {
        let team1 = Team::from_challonge(&mut *transaction, &set.relationships.player1.data.id).await?.ok_or_else(|| cal::Error::UnknownTeamChallonge(set.relationships.player1.data.id.clone()))?;
        let team2 = Team::from_challonge(&mut *transaction, &set.relationships.player2.data.id).await?.ok_or_else(|| cal::Error::UnknownTeamChallonge(set.relationships.player2.data.id.clone()))?;
        races.push(Race {
            id: Id::new(transaction).await?,
            series: event.series,
            event: event.event.to_string(),
            source: cal::Source::Challonge { id: set.id },
            entrants: Entrants::Two([
                Entrant::MidosHouseTeam(team1.clone()),
                Entrant::MidosHouseTeam(team2.clone()),
            ]),
            phase: None,
            round: None,
            game: None,
            scheduling_thread: None,
            schedule: RaceSchedule::Unscheduled,
            schedule_updated_at: None,
            draft: None, //TODO reuse code from startgg::races_to_import
            seed: seed::Data::default(),
            video_urls: HashMap::default(),
            restreamers: HashMap::default(),
            last_edited_by: None,
            last_edited_at: None,
            ignored: false,
            schedule_locked: false,
        });
    }
    Ok(races)
}
