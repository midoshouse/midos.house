...
pub(crate) async fn merge_accounts(global: &GlobalState, me: User, racetime_user: Option<RaceTimeUser>, discord_user: Option<DiscordUser>) -> Result<Redirect, MergeAccountsError> {
    ...
    match (me.racetime.is_some(), me.discord.is_some()) {
        ...
        (is_racetime, _) => if let Some(discord_user) = discord_user {
            if is_racetime
                && let Ok(Some(to_merge)) = User::from_discord(&mut *transaction, discord_user.id).await
                && to_merge.racetime.is_none()
                && let discord = to_merge.discord.expect("Discord user without Discord ID")
                && sqlx::query!("DELETE FROM users WHERE id = $1", to_merge.id as _).execute(&mut *transaction).await.is_ok()
