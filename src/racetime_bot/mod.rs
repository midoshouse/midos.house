...
pub(crate) enum Error {
    #[error(transparent)] Auth(#[from] racetime::AuthError),
    ...
    #[error(transparent)] Edit(#[from] racetime::EditError),
    #[error(transparent)] EventData(#[from] event::DataError),
    #[error(transparent)] GoalFromStr(#[from] GoalFromStrError),
    ...
    #[error(transparent)] Server(#[from] racetime::handler::ServerErrors),
...
impl Goal {
    ...
    fn single_event(&self) -> Result<(Series, &'static str), fn(Series, &str) -> bool> {
        match self {
            ...
            Self::LeagueS4 => Ok((Series::League, "4")),
            ...
            Self::TournoiFrancoS6 => Ok((Series::TournoiFrancophone, "6")),
    ...
    pub(crate) fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::TournoiFrancoS6 => "Tournoi Francophone Saison 6",
    ...
    fn draft_kind(&self) -> Option<draft::Kind> {
        match self {
            ...
            Self::TournoiFrancoS6 => Some(draft::Kind::TournoiFrancoS6),
            | Self::BattleRoyaleS1
            | Self::BattleRoyaleS2
            | Self::CoOpS3
            | Self::CopaDoBrasil
    ...
    pub(crate) fn rando_version(&self) -> Result<VersionedBranch, &'static str> {
        Ok(match self {
            ...
            Self::LeagueS4 => VersionedBranch::Pinned { version: rando::Version::from_dev(7, 1, 143) },
            ...
            Self::TournoiFrancoS6 => VersionedBranch::Pinned { version: rando::Version::from_branch(rando::Branch::DevRob, 9, 0, 2, 17) },
    ...
    pub(crate) fn single_settings(&self) -> Option<seed::Settings> {
        match self {
            ...
            Self::LeagueS4 => Some(league::s4_settings()),
            ...
            Self::TournoiFrancoS6 => None, // settings draft
    ...
    async fn send_presets(&self, ctx: &RaceContext<GlobalState>) -> Result<(), Error> {
        match self {
            ...
            Self::TournoiFrancoS4 | Self::TournoiFrancoS5 | Self::TournoiFrancoS6 => {
    ...
    pub(crate) async fn parse_seed_command(&self, transaction: &mut Transaction<'_, Postgres>, global: &GlobalState, is_official: bool, spoiler_seed: bool, no_password: bool, args: &[String]) -> Result<SeedCommandParseResult, Error> {
        ...
        Ok(match self {
            ...
            | Self::WeTryToBeBetterS2
                => {
                    ...
                    if let Some(row) = sqlx::query!(r#"DELETE FROM prerolled_seeds WHERE ctid IN (SELECT ctid FROM prerolled_seeds WHERE goal_name = $1 AND (seed_password IS NULL OR NOT $2) ORDER BY timestamp ASC NULLS FIRST LIMIT 1) RETURNING
                        ...
                        bingosync_url,
                        bingo_passphrase
                    "#, self.as_str(), no_password).fetch_optional(&mut **transaction).await? {
            ...
            Self::Cc7 | Self::S7 => {
                let settings = match args {
                    ...
                    }.complete_randomly(draft::Kind::S7).await?,
            ...
            Self::MultiworldS2 | Self::MultiworldS3 | Self::MultiworldS4 | Self::MultiworldS5 | Self::MultiworldS6 => {
                ...
                let settings = match args {
                    ...
                    [arg] if arg == "random" => Draft {
                        ...
                        skipped_bans: 0,
                        settings: HashMap::default(),
                    }.complete_randomly(self.draft_kind().expect("multiworld tournament goal should have a draft kind")).await?,
                    [arg] if arg == "draft" => return Ok(SeedCommandParseResult::StartDraft {
                        kind: self.draft_kind().expect("missing draft kind"),
                        new_state: Draft {
                            high_seed: Id::dummy(), // racetime.gg bot doesn't check for active team
            ...
            Self::Rsl => {
                let (preset, world_count) = match args {
                    ...
                    [preset] if preset == "s3" => return Ok(SeedCommandParseResult::Rsl {
                        preset: rsl::VersionedPreset::Xopar { version: Some(Version::new(2, 1, 0)), preset: rsl::Preset::League },
                        world_count: 1,
                        language: English,
                        article: "an",
                        description: format!("RSL season 3 seed"),
                        unlock_spoiler_log,
                    }),
            ...
            Self::SlugOpen2026 => {
                ...
                match format.draft_kind() {
                    None => {
                        let (rando_version, settings, bingo_passphrase) = format.single_settings(global, Some(&format!("{} Practice: {}", self.as_str(), format.display_name()))).await?.ok_or(Error::NoSingleSettings)?;
                        SeedCommandParseResult::Regular {
            ...
            Self::StandardRuleset => match args {
                ...
                [arg] if arg == "weekly" => {
                    //TODO offer rolling archived weekly settings?
                    SeedCommandParseResult::SendPresets { language: English, msg: "weekly settings are no longer available" }
                }
                [..] => SeedCommandParseResult::SendPresets { language: English, msg: "I didn't quite understand that" },
            },
            Self::TournoiFrancoS3 | Self::TournoiFrancoS4 | Self::TournoiFrancoS5 | Self::TournoiFrancoS6 => {
                let all_settings = match self {
                    ...
                    Self::TournoiFrancoS6 => &fr::S6_SETTINGS[..],
                    _ => unreachable!(),
                };
                let mut args = args.to_owned();
                let mut mq_dungeons_count = None::<u8>;
                ...
                let settings = match &*args {
                    ...
                    }.complete_randomly(self.draft_kind().unwrap()).await?,
                    [arg] if arg == "draft" => return Ok(SeedCommandParseResult::StartDraft {
                        kind: self.draft_kind().expect("missing draft kind"),
                        new_state: Draft {
                            high_seed: Id::dummy(), // racetime.gg bot doesn't check for active team
            ...
            Self::TriforceBlitz => match args {
                ...
                [arg] if arg == "midnight" => SeedCommandParseResult::Tfb { display_name: "Season 5 (Co-op)", preset: Some("Triforce Blitz S5 Midnight"), unlock_spoiler_log, language: English, article: "a", description: format!("Triforce Blitz: Midnight seed") },
                [arg] if arg == "jr" => SeedCommandParseResult::Tfb { display_name: "Season 2 - Jabu's Revenge", preset: None, unlock_spoiler_log, language: English, article: "a", description: format!("Triforce Blitz: Jabu's Revenge seed") },
                ...
                [arg, preset] if arg == "daily" => {
                    ...
                    if let Some(entry) = response.entries.into_iter().find(|entry| entry.preset_key == preset_key) {
                        SeedCommandParseResult::QueueExisting { data: seed::Data {
                            ...
                            files: Some(seed::Files::TriforceBlitz {
                                is_dev: false,
                                uuid: entry.seed.id,
                                scheduled_spoiler_unlock: Some(entry.scheduled_spoiler_unlock_at_utc),
                            }),
                            progression_spoiler: false,
                        }, language: English, article: "the", description: format!("Triforce Blitz seed of the day ({})", entry.preset_display_name) }
                    } else {
                        SeedCommandParseResult::Error { language: English, msg: "this TFB Seed of the Day preset does not appear to be available".into() }
                    }
                }
                [..] => SeedCommandParseResult::SendPresets { language: English, msg: "I didn't quite understand that" },
            },
...
impl GlobalState {
    ...
    pub(crate) fn roll_tfb_seed(self: Arc<Self>, delay_until: Option<DateTime<Utc>>, display_name: &'static str, settings_preset: Option<&'static str>, room: Option<String>, password_lock: bool, unlock_spoiler_log: UnlockSpoilerLog) -> mpsc::Receiver<SeedRollUpdate> {
        ...
        tokio::spawn(async move {
            ...
            update_tx.send(SeedRollUpdate::Started).await.allow_unreceived();
            let seed_request = match (unlock_spoiler_log, &room) {
...
pub(crate) async fn roll_seed_locally(delay_until: Option<DateTime<Utc>>, version: VersionedBranch, unlock_spoiler_log: bool, mut settings: seed::Settings, plando: serde_json::Map<String, serde_json::Value>) -> Result<(String, Option<PathBuf>), RollError> {
    ...
    let rando_path = match version {
        VersionedBranch::Pinned { ref version } => {
            version.clone_repo(allow_riir, false).await?;
            version.dir(allow_riir)?
        }
        VersionedBranch::Latest { branch } => {
            branch.clone_repo(allow_riir, false).await?;
...
pub(crate) enum RollError {
    ...
    ChannelClosed,
    #[error("attempted to roll seed for unfinished settings draft")]
    Draft,
...
impl SeedRollUpdate {
    async fn handle(self, global: &GlobalState, ctx: &RaceContext<GlobalState>, state: &ArcRwLock<RaceState>, official_data: Option<&OfficialRaceData>, language: Language, article: &'static str, description: &str) -> Result<(), Error> {
        match self {
            ...
            Self::Done { mut seed, rsl_preset, unlock_spoiler_log } => {
                ...
                if let Some(OfficialRaceData { cal_event, .. }) = official_data {
                    match seed.files.as_ref().expect("received seed with no files") {
                        ...
                        seed::Files::TriforceBlitz { is_dev, uuid, scheduled_spoiler_unlock: _ } => {
                            sqlx::query!(
                    ...
                    if let Some(password) = &extra.password {
                        sqlx::query!("UPDATE races SET seed_password = $1 WHERE id = $2", password.iter().copied().map(char::from).collect::<String>(), cal_event.race.id as _).execute(&global.db_pool).await?;
                    }
                    if let Some(seed::BingoData { url, passphrase }) = &extra.bingo {
                        sqlx::query!("UPDATE races SET bingosync_url = $1, bingo_passphrase = $2 WHERE id = $3", url.as_str(), passphrase, cal_event.race.id as _).execute(&global.db_pool).await?;
                    }
                }
                ...
                match unlock_spoiler_log {
                    ...
                    UnlockSpoilerLog::After => if let Some(seed::Files::TriforceBlitz { scheduled_spoiler_unlock: Some(unlock_time), .. }) = seed.files {
                        ...
                        if let Entrant::MidosHouseTeam(team) = entrant && let Some(mw::Impl::MidosHouse) = team.mw_impl {
                            ...
                            if let Some(hash) = extra.file_hash {
                                ...
                                if let Some(tracker_room_name) = tracker_room_name {
                                    ...
                                    if !all_notified {
                                        FENHL.create_dm_channel(discord_ctx!(ctx.global_state)).await?.say(discord_ctx!(ctx.global_state), format!("auto-tracker room for {reply_to}: `{tracker_room_name}`")).await?;
                                    }
                                }
                                mw_rooms_created += 1;
                            } else {
            ...
            Self::Error(e) => {
                ...
                if let Environment::Production = Environment::default() {
                    wheel::night_report(&format!("{}/error", night_path()), Some(&format!("seed roll error in https://{}{}: {e} ({e:?})", racetime_host(), ctx.data().await.url))).await?;
                }
...
async fn room_options(goal: Goal, event: &event::Data<'_>, cal_event: &cal::Event, info_user: String, info_bot: String, auto_start: bool) -> racetime::StartRace {
    racetime::StartRace {
        ...
        partitionable: false,
        hide_entrants: event.series == Series::BattleRoyale && cal_event.race.entrants == Entrants::Open,
        ...
        start_delay: if
            cal_event.race.entrants == Entrants::Open
            || goal.unlock_spoiler_log(true, false) == UnlockSpoilerLog::Never && goal.rando_version().is_ok_and(|rando_version| matches!(rando_version, VersionedBranch::Pinned { version } if version.is_release() && (version.base().major, version.base().minor) >= (9, 1))) // longer countdowns for 10-button passwords
        { 30 } else { 15 },
        time_limit: 24,
...
impl Handler {
    ...
    async fn send_settings(&self, ctx: &RaceContext<GlobalState>, preface: &str, reply_to: &str) -> Result<(), Error> {
        ...
        if let Some(draft_kind) = self.draft_kind(goal).await {
            let available_settings = lock!(@read state = self.race_state; if let RaceState::Draft { state: ref draft, .. } = *state {
                match draft.next_step(draft_kind, self.official_data.as_ref().and_then(|OfficialRaceData { cal_event, .. }| cal_event.race.game), &mut draft::MessageContext::RaceTime { high_seed_name: &self.high_seed_name, low_seed_name: &self.low_seed_name, reply_to }).await?.kind {
            ...
            let available_settings = available_settings.unwrap_or_else(|| match draft_kind {
                ...
                draft::Kind::TournoiFrancoS6 => fr::S6_SETTINGS.into_iter().map(|fr::Setting { description, .. }| Cow::Borrowed(description)).collect(),
            });
            ...
            if let Some(draft_kind) = self.draft_kind(goal).await {
                lock!(@write state = self.race_state; match *state {
                    RaceState::Init => match draft_kind {
                        ...
                        draft::Kind::TournoiFrancoS4 | draft::Kind::TournoiFrancoS5 | draft::Kind::TournoiFrancoS6 => ctx.say(format!("Sorry {reply_to}, no draft has been started. Use “!seed draft” to start one. For more info about these options, use !presets / le draft n'a pas débuté. Utilisez “!seed draft” pour en commencer un. Pour plus d'infos, utilisez !presets")).await?,
                    },
                    RaceState::Draft { state: ref mut draft, .. } => {
                        let is_active_team = if let Some(OfficialRaceData { ref cal_event, ref event, .. }) = self.official_data {
                            ...
                            let is_active_team = if let Some(sender) = sender
                                && let Some(user) = User::from_racetime(&mut *transaction, &sender.id).await?
                                && let Some(team) = Team::from_event_and_member(&mut transaction, event.series, &event.event, user.id).await?
                            {
                                draft.is_active_team(draft_kind, cal_event.race.game, team.id).await?
                            } else {
                                false
                            };
                            transaction.commit().await?;
                        ...
                        if is_active_team {
                            match draft.apply(draft_kind, self.official_data.as_ref().and_then(|OfficialRaceData { cal_event, .. }| cal_event.race.game), &mut draft::MessageContext::RaceTime { high_seed_name: &self.high_seed_name, low_seed_name: &self.low_seed_name, reply_to }, action).await? {
                            ...
                            match draft_kind {
                                ...
                                draft::Kind::TournoiFrancoS4 | draft::Kind::TournoiFrancoS5 | draft::Kind::TournoiFrancoS6 => ctx.say(format!("Sorry {reply_to}, it's not your turn in the settings draft. / mais ce n'est pas votre tour.")).await?,
                            }
                        }
                    }
    ...
    async fn roll_seed_inner(&self, ctx: &RaceContext<GlobalState>, delay_until: Option<DateTime<Utc>>, mut updates: mpsc::Receiver<SeedRollUpdate>, language: Language, article: &'static str, description: String) {
        let global = ctx.global_state.clone();
        let ctx = ctx.clone();
                    ...
                    select! {
                        () = &mut sleep => {
                            if let Some(update) = seed_state.take() {
                                update.handle(&global, &ctx, &state, official_data.as_ref(), language, article, &description).await?;
                            }
                    ...
                    if let UnlockSpoilerLog::Progression | UnlockSpoilerLog::After = goal.unlock_spoiler_log(self.is_official(), false /* we may try to unlock a log that's already unlocked, but other than that, this assumption doesn't break anything */) {
                        match files {
                            seed::Files::MidosHouse { file_stem, locked_spoiler_log_path } => if let Some(locked_spoiler_log_path) = locked_spoiler_log_path {
                                fs::rename(locked_spoiler_log_path, Path::new(seed::DIR).join(format!("{file_stem}_Spoiler.json"))).await?;
                                sqlx::query!("UPDATE seed_metadata SET locked_spoiler_log_path = NULL WHERE file_stem = $1", file_stem).execute(&ctx.global_state.db_pool).await?;
                            },
                            seed::Files::OotrWeb { id, file_stem, .. } => {
                                ctx.global_state.ootr_api_client.unlock_spoiler_log(*id).await?;
                                let spoiler_log = ctx.global_state.ootr_api_client.seed_details(*id).await?.spoiler_log;
                                fs::write(Path::new(seed::DIR).join(format!("{file_stem}_Spoiler.json")), &spoiler_log).await?;
                            }
...
impl SeedHandler for Handler {
    ...
    async fn roll_tfb_seed(&self, ctx: &RaceContext<GlobalState>, display_name: &'static str, preset: Option<&'static str>, password_lock: bool, unlock_spoiler_log: UnlockSpoilerLog, language: Language, article: &'static str, description: String) {
        let official_start = self.official_data.as_ref().map(|official_data| official_data.cal_event.start().expect("handling room for official race without start time"));
        let delay_until = official_start.map(|start| start - TimeDelta::minutes(15));
        ...
        self.roll_seed_inner(ctx, delay_until, ctx.global_state.clone().roll_tfb_seed(delay_until, display_name, preset, Some(format!("https://{}{}", racetime_host(), ctx.data().await.url)), password_lock, unlock_spoiler_log), language, article, description).await;
    }
    ...
    async fn advance_draft(&self, ctx: &RaceContext<GlobalState>, state: &RaceState) -> Result<(), Error> {
        ...
        let step = draft.next_step(kind, self.official_data.as_ref().and_then(|OfficialRaceData { cal_event, .. }| cal_event.race.game), &mut draft::MessageContext::RaceTime { high_seed_name: &self.high_seed_name, low_seed_name: &self.low_seed_name, reply_to: "friend" }).await?;
        match step.kind {
            draft::StepKind::Done(settings) => {
                ...
                self.roll_seed(ctx, goal.preroll_seeds(), kind.rando_version().unwrap_or_else(|| goal.rando_version().unwrap()), settings, serde_json::Map::default(), None, unlock_spoiler_log, goal.language(), article, description).await;
            }
...
impl RaceHandler<GlobalState> for Handler {
        ...
        tokio::spawn(async move {
            ...
            lock!(@read data = race_data; {
                ...
                match res {
                    Ok(Ok(())) => {}
                    Ok(Err(e)) => {
                        eprintln!("race handler for https://{}{} errored: {e}\ndebug info: {e:?}", racetime_host(), data.url);
                        if let Environment::Production = Environment::default() {
                            let _ = wheel::night_report(&format!("{}/error", night_path()), Some(&format!("race handler for https://{}{} errored: {e}\ndebug info: {e:?}", racetime_host(), data.url))).await;
                        }
                    }
                    Err(e) => {
                        eprintln!("race handler for https://{}{} panicked: {e}\ndebug info: {e:?}", racetime_host(), data.url);
                        if let Environment::Production = Environment::default() {
                            let _ = wheel::night_report(&format!("{}/error", night_path()), Some(&format!("race handler for https://{}{} panicked: {e}\ndebug info: {e:?}", racetime_host(), data.url))).await;
                        }
                    }
                }
            });
    ...
    async fn new(ctx: &RaceContext<GlobalState>) -> Result<Self, Error> {
        ...
        let (existing_seed, official_data, race_state, high_seed_name, low_seed_name, fpa_enabled) = lock!(new_room_lock = ctx.global_state.new_room_lock; { // make sure a new room isn't handled before it's added to the database
            ...
            let new_data = if let Some(cal_event) = cal::Event::from_room(&mut transaction, &ctx.global_state.http_client, format!("https://{}{}", racetime_host(), ctx.data().await.url).parse()?).await? {
                ...
                for member in cal_event.racetime_users_to_invite(&mut transaction, discord_ctx!(ctx.global_state), &event).await? {
                    match member {
                        Ok(member) => {
                ...
                let (race_state, high_seed_name, low_seed_name) = if let Some(kind) = cal_event.race.draft_kind(&event) {
                    ...
                    let [high_seed_name, low_seed_name] = if let draft::StepKind::Done(_) | draft::StepKind::DoneRsl { .. } = state.next_step(kind, cal_event.race.game, &mut draft::MessageContext::None).await?.kind {
                        ...
                        match cal_event.race.entrants {
                            ...
                            Entrants::Two([Entrant::MidosHouseTeam(ref team1), Entrant::MidosHouseTeam(ref team2)]) => {
                                let name1 = if let Ok(member) = team1.members(&mut transaction).await?.into_iter().exactly_one()
                                    && let Some(ref racetime) = member.racetime
                                {
                                    racetime.display_name.clone()
                                } else {
                                    team1.name(&mut transaction).await?.map_or_else(|| format!("Team A"), Cow::into_owned)
                                };
                                let name2 = if let Ok(member) = team2.members(&mut transaction).await?.into_iter().exactly_one()
                                    && let Some(ref racetime) = member.racetime
                                {
                                    racetime.display_name.clone()
                                } else {
                                    team2.name(&mut transaction).await?.map_or_else(|| format!("Team B"), Cow::into_owned)
                                };
                ...
                for (&language, video_url) in &cal_event.race.video_urls {
                    restreams.insert(video_url.clone(), RestreamState {
                        ...
                        restreamer_racetime_id: if let Some(restreamer) = cal_event.race.restreamers.get(&language) {
                            Some(restreamer.racetime_id(&mut transaction).await?)
                        } else {
                            None
                        },
                        ready: false,
                ...
                if let Some(ref info_bot) = data.info_bot {
                    for section in info_bot.split(" | ") {
                        if let Some((_, file_stem)) = regex_captures!(r"^Seed: https://midos\.house/seed/(.+)(?:\.zpfz?)?$", section) {
                            ...
                            let id = seed_id.parse()?;
                            let file_stem = ctx.global_state.ootr_api_client.patch_file_stem(id).await?;
                            race_state = RaceState::Rolled(seed::Data {
                                file_hash: None,
                ...
                if let RaceStatusValue::Pending | RaceStatusValue::InProgress = data.status.value { //TODO also check this in official races
                    ...
                    match race_state {
                        RaceState::Init => match goal {
                            ...
                            Goal::TournoiFrancoS6 => ctx.send_message( //TODO post welcome message in both English and French
                                "Welcome! This is a practice room for the Tournoi Francophone Saison 6. Learn more about the tournament at https://midos.house/event/fr/6",
                                true,
                                vec![
                                    ("Roll seed (base settings)", ActionButton::Message {
                                        message: format!("!seed base ${{mq}}mq"),
                            ...
                            Goal::TriforceBlitz => ctx.send_message(
                                ...
                                vec![
                                    ...
                                    */
                                    ...
                                    ("Roll midnight (1v1) seed", ActionButton::Message {
                                        message: format!("!seed midnight"),
                                        help_text: Some(format!("Create a Triforce Blitz season 5 Midnight 1v1 seed.")),
                                        survey: None,
                                        submit: None,
                                    }),
                                    ("Seed of the Day", ActionButton::Message {
                                        message: format!("!seed daily ${{preset}}"),
        ...
        match &*cmd_name.to_ascii_lowercase() {
            ...
            "fpa" => match args[..] {
                ...
                [ref arg] => match &*arg.to_ascii_lowercase() {
                    ...
                    } else if !self.can_monitor(ctx, is_monitor, msg).await? {
                        ctx.say(if let French = goal.language() {
            ...
            "lock" => if self.can_monitor(ctx, is_monitor, msg).await? {
                self.locked = true;
                ctx.say(if let French = goal.language()
                    && !self.is_official()
                {
            ...
            "monitor" => if self.can_monitor(ctx, is_monitor, msg).await? {
            ...
            "ready" => if let Some(OfficialRaceData { ref mut restreams, ref cal_event, ref event, .. }) = self.official_data {
                ...
                if restreams.values().all(|state| state.ready) {
                    ...
                    let (access_token, _) = racetime::authorize_with_host(&racetime_host_info(), &ctx.global_state.config.racetime_bot.client_id, &ctx.global_state.config.racetime_bot.client_secret, &ctx.global_state.http_client).await?;
                    room_options(
                        goal, event, cal_event,
                        ctx.data().await.info_user.clone().unwrap_or_default(),
                        ctx.data().await.info_bot.clone().unwrap_or_default(),
                        true,
                    ).await.edit_with_host(&racetime_host_info(), &access_token, &ctx.global_state.http_client, CATEGORY, &ctx.data().await.slug).await?;
                } else {
            ...
            "restreamer" => if self.can_monitor(ctx, is_monitor, msg).await? {
            ...
            "seed" | "spoilerseed" => if let RaceStatusValue::Open | RaceStatusValue::Invitational = ctx.data().await.status.value {
                lock!(@write state = self.race_state; match *state {
                    RaceState::Init => if self.locked && !self.can_monitor(ctx, is_monitor, msg).await? {
                        ctx.say(if let French = goal.language() {
                        ...
                        let mut transaction = ctx.global_state.db_pool.begin().await?;
                        match goal.parse_seed_command(&mut transaction, &ctx.global_state, self.is_official(), cmd_name.eq_ignore_ascii_case("spoilerseed"), false, &args).await? {
                            SeedCommandParseResult::Regular { rando_version_override, settings, plando, bingo_passphrase, unlock_spoiler_log, language, article, description } => self.roll_seed(ctx, goal.preroll_seeds(), rando_version_override.unwrap_or_else(|| goal.rando_version().unwrap()), settings, plando, bingo_passphrase, unlock_spoiler_log, language, article, description).await,
            ...
            "unlock" => if self.can_monitor(ctx, is_monitor, msg).await? {
                self.locked = false;
                ctx.say(if let French = goal.language() {
    ...
    async fn race_data(&mut self, ctx: &RaceContext<GlobalState>, _old_race_data: RaceData) -> Result<(), Error> {
        ...
        if !self.start_saved {
            if let (Goal::Rsl, Some(start)) = (goal, data.started_at) {
                sqlx::query!("UPDATE rsl_seeds SET start = $1 WHERE room = $2", start, format!("https://{}{}", racetime_host(), ctx.data().await.url)).execute(&ctx.global_state.db_pool).await?;
                        ...
                        if let Some(OfficialRaceData { cal_event, event, .. }) = &self.official_data {
                            if event.series == Series::Standard && cal_event.race.entrants == Entrants::Open && event.discord_guild == Some(discord_bot::OOTR_GUILD) {
                                ...
                                let channel = match &*event.event {
                                    ...
                                    _ => {
                                        if let Environment::Production = Environment::default() {
                                            wheel::night_report(&format!("{}/error", night_path()), Some(&format!("no password channel configured for event s/{}", event.event))).await?;
                                        }
                                        None
                                    }
                                };
                            ...
                            if !cleaned_up.load(atomic::Ordering::SeqCst) {
                                if let Some((event, cal_event)) = official_data {
                                    if let Some(organizer_channel) = event.discord_organizer_channel {
                                        ...
                                        let _ = organizer_channel.say(discord_ctx!(ctx.global_state), msg.build()).await;
                                    }
                                }
                            }
                        }));
    ...
    async fn error(&mut self, _: &RaceContext<GlobalState>, mut errors: Vec<String>) -> Result<(), Error> {
        ...
        if errors.is_empty() {
            ...
            Err(Error::Server(racetime::handler::ServerErrors(errors)))
        }
    }
}
...
pub(crate) async fn create_room(transaction: &mut Transaction<'_, Postgres>, global: &Arc<GlobalState>, host_info: &racetime::HostInfo, cal_event: &mut cal::Event, event: &event::Data<'_>) -> Result<Option<(bool, String, Option<CreateAllowedMentions>)>, Error> {
    let room_url = match cal_event.should_create_room(&mut *transaction, event).await? {
        ...
        RaceHandleMode::RaceTime => match racetime::authorize_with_host(host_info, &global.config.racetime_bot.client_id, &global.config.racetime_bot.client_secret, &global.http_client).await {
            Ok((access_token, _)) => {
                let info_user = if let French = event.language
                    ...
                    && let Some(entrants) = match cal_event.race.entrants {
                        ...
                        Entrants::Two([ref team1, ref team2]) => match cal_event.kind {
                            cal::EventKind::Normal => if let (Some(team1), Some(team2)) = (team1.name(&mut *transaction, discord_ctx!(global)).await?, team2.name(&mut *transaction, discord_ctx!(global)).await?) {
                        ...
                        Entrants::Three([ref team1, ref team2, ref team3]) => match cal_event.kind {
                            cal::EventKind::Normal => if let (Some(team1), Some(team2), Some(team3)) = (team1.name(&mut *transaction, discord_ctx!(global)).await?, team2.name(&mut *transaction, discord_ctx!(global)).await?, team3.name(&mut *transaction, discord_ctx!(global)).await?) {
                    ...
                    let mut info_user = match cal_event.race.entrants {
                        ...
                        Entrants::Two([ref team1, ref team2]) => match cal_event.kind {
                            cal::EventKind::Normal => format!(
                                ...
                                team1.name(&mut *transaction, discord_ctx!(global)).await?.unwrap_or(Cow::Borrowed("(unnamed)")),
                                team2.name(&mut *transaction, discord_ctx!(global)).await?.unwrap_or(Cow::Borrowed("(unnamed)")),
                            ),
                        ...
                        Entrants::Three([ref team1, ref team2, ref team3]) => match cal_event.kind {
                            cal::EventKind::Normal => format!(
                                ...
                                team1.name(&mut *transaction, discord_ctx!(global)).await?.unwrap_or(Cow::Borrowed("(unnamed)")),
                                team2.name(&mut *transaction, discord_ctx!(global)).await?.unwrap_or(Cow::Borrowed("(unnamed)")),
                                team3.name(&mut *transaction, discord_ctx!(global)).await?.unwrap_or(Cow::Borrowed("(unnamed)")),
                            ),
                            cal::EventKind::Async1 => format!(
                                "{} (async): {} vs {} vs {}",
                                info_prefix.clone().unwrap_or_default(),
                ...
                *cal_event.room_mut().expect("opening room for official race without start time") = Some(room_url.clone());
                lock!(@read extra_room_tx = global.extra_room_tx; extra_room_tx.send(race_slug).await.allow_unreceived());
                Ok(room_url)
            }
            Err(racetime::AuthError::ResponseStatus { inner, .. }) if inner.status().is_some_and(|status| status.is_server_error()) => {
        ...
        RaceHandleMode::AsyncForm => {
            ...
            impl SeedHandler for AsyncSeedHandler {
                ...
                async fn roll_tfb_seed(&self, global: &Arc<GlobalState>, display_name: &'static str, preset: Option<&'static str>, password_lock: bool, unlock_spoiler_log: UnlockSpoilerLog, _: Language, _: &'static str, _: String) -> mpsc::Receiver<SeedRollUpdate> {
                    let delay_until = Some(self.start - TimeDelta::minutes(15));
                    global.clone().roll_tfb_seed(delay_until, display_name, preset, None, password_lock, unlock_spoiler_log)
                }
                ...
                async fn advance_draft(&self, global: &Arc<GlobalState>, state: &RaceState) -> Result<Self::SeedResult, Error> {
                    let RaceState::Draft { kind, state: ref draft, unlock_spoiler_log } = *state else { unreachable!() };
                    let step = draft.next_step(kind, self.race.game, &mut draft::MessageContext::RaceTime { high_seed_name: "Team A", low_seed_name: "Team B", reply_to: "friend" }).await?;
                    Ok(match step.kind {
                        draft::StepKind::Done(settings) => {
                            ...
                            self.roll_seed(global, self.goal.preroll_seeds(), kind.rando_version().unwrap_or_else(|| self.goal.rando_version().unwrap()), settings, serde_json::Map::default(), None, unlock_spoiler_log, self.goal.language(), article, description).await
                        }
                        draft::StepKind::DoneRsl { preset, world_count } => {
                            let (article, description) = if let French = self.goal.language() {
                                ("une", format!("seed avec {}", step.message))
            ...
            if let Some(mut rx) = (AsyncSeedHandler {
                ...
                while let Some(update) = rx.recv().await {
                    match update {
                        SeedRollUpdate::Queued(_) | SeedRollUpdate::MovedForward(_) | SeedRollUpdate::Started => {}
                        ...
                        SeedRollUpdate::Done { mut seed, rsl_preset: _, unlock_spoiler_log: _ } => {
                            ...
                            if !cal_event.race.has_any_room() {
                                cal_event.race.seed = seed;
                                cal_event.race.save(&mut *transaction).await?;
                            }
                            msg = MessageBuilder::default()
                                .push("go to ")
                                .push_named_link_no_preview("the race page", uri!(base_uri(), cal::race_details(event.series, &*event.event, cal_event.race.id)).to_string())
                                .push(" to request the seed")
                                .build()
                                .into();
                            break
                        }
                        ...
                        SeedRollUpdate::Error(e) => {
                            eprintln!("seed roll error for async: {e} ({e:?})");
                            msg = "there was an error rolling your seed, please ask an organizer to roll it manually".into();
                            break
                        }
                    }
                }
                ...
                Err("there was an error rolling your seed, please ask an organizer to roll it manually".into())
            }
        }
    };
    ...
    let (msg, allowed_mentions) = if let French = event.language
        ...
        && let (Some(phase), Some(round)) = (cal_event.race.phase.as_ref(), cal_event.race.round.as_ref())
        && sqlx::query_scalar!(r#"SELECT EXISTS (SELECT 1 FROM phase_round_options WHERE series = $1 AND event = $2 AND phase = $3 AND round = $4 AND display_fr IS NOT NULL) AS "exists!""#, event.series as _, &event.event, phase, round).fetch_one(&mut **transaction).await?
        && cal_event.race.game.is_none()
    {
        let mut msg = MessageBuilder::default();
        ...
        msg.push(" : ");
        let allowed_mentions = cal_event.format_discord(&mut *transaction, event.discord_guild, French, &mut msg).await?;
        msg.push(" <");
        ...
        msg.push_timestamp(cal_event.start().expect("opening room for official race without start time"), serenity_utils::message::TimestampStyle::Relative);
        msg.push(": ");
        let allowed_mentions = cal_event.format_discord(&mut *transaction, event.discord_guild, English, &mut msg).await?;
        match room_url {
            Ok(room_url) => {
                msg.push(' ');
...
async fn prepare_seeds(global: Arc<GlobalState>, mut seed_cache_rx: watch::Receiver<()>, mut shutdown: rocket::Shutdown) -> Result<(), PrepareSeedsError> {
    'outer: loop {
        for id in sqlx::query_scalar!(r#"SELECT id AS "id: Id<Races>" FROM races WHERE NOT ignored AND room IS NULL AND async_room1 IS NULL AND async_room2 IS NULL AND async_room3 IS NULL AND file_stem IS NULL AND tfb_uuid IS NULL"#).fetch_all(&global.db_pool).await? {
            let mut transaction = global.db_pool.begin().await?;
        ...
        let event_rows = sqlx::query!(r#"SELECT series AS "series: Series", event FROM events WHERE end_time IS NULL OR end_time > NOW()"#).fetch_all(&global.db_pool).await?;
        for goal in all::<Goal>() {
            if let Some(settings) = goal.single_settings() {
                if goal.preroll_seeds() == PrerollMode::Long && event_rows.iter().any(|row| goal.matches_event(row.series, &row.event)) {
                    if sqlx::query_scalar!(r#"SELECT EXISTS (SELECT 1 FROM prerolled_seeds WHERE goal_name = $1) AS "exists!""#, goal.as_str()).fetch_one(&global.db_pool).await? { break }
                    'seed: loop {
                        let mut seed_rx = global.clone().roll_seed(
                            PrerollMode::Long,
                            false,
                            None,
                            goal.rando_version().unwrap(),
                            settings.clone(),
                            serde_json::Map::default(),
                            None,
...
async fn create_rooms(global: Arc<GlobalState>, mut shutdown: rocket::Shutdown) -> Result<(), CreateRoomsError> {
    loop {
        select! {
            () = &mut shutdown => break,
            _ = sleep(Duration::from_secs(30)) => { //TODO exact timing (coordinate with everything that can change the schedule)
                lock!(new_room_lock = global.new_room_lock; { // make sure a new room isn't handled before it's added to the database
                    let mut transaction = global.db_pool.begin().await?;
...
pub(crate) enum HandleRoomsError {
    #[error(transparent)] Auth(#[from] racetime::AuthError),
    #[error(transparent)] Run(#[from] racetime::bot::RunError<Error>),
    #[error(transparent)] Wheel(#[from] wheel::Error),
}

async fn handle_rooms(global: Arc<GlobalState>, shutdown: rocket::Shutdown) -> Result<(), HandleRoomsError> {
    let mut last_crash = Instant::now();
    let mut wait_time = Duration::from_secs(1);
    loop {
        match racetime::BotBuilder::new(CATEGORY, &global.config.racetime_bot.client_id, &global.config.racetime_bot.client_secret)
            .state(global.clone())
        ...
        {
            Ok(bot) => {
                lock!(@write extra_room_tx = global.extra_room_tx; *extra_room_tx = bot.extra_room_sender());
...
pub(crate) async fn main(global: Arc<GlobalState>, shutdown: rocket::Shutdown, seed_cache_rx: watch::Receiver<()>) -> Result<(), MainError> {
    let ((), (), ()) = tokio::try_join!(
        prepare_seeds(global.clone(), seed_cache_rx, shutdown.clone()).err_into::<MainError>(),
        create_rooms(global.clone(), shutdown.clone()).err_into(),
        handle_rooms(global, shutdown).err_into(),
    )?;
    Ok(())
}
