    ...
    let form = match event.match_source() {
        ...
        MatchSource::Challonge { community, tournament } => if me.is_some() {
            ...
            if races.is_empty() {
                ...
                let table = race_table(&mut transaction, discord_ctx, env, http_client, Some(&event), RaceTableOptions { game_count: true, show_multistreams: false, can_create: false, can_edit: false, show_restream_consent: false, challonge_import_ctx: Some(ctx) }, &races).await?;
