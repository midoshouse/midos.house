...
async fn check_scheduling_thread_permissions<'a>(ctx: &'a Context, interaction: &ApplicationCommandInteraction) -> Result<Option<(Transaction<'a, Postgres>, Race, Option<Team>)>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    Ok(if let Some(race) = Race::for_scheduling_channel(&mut transaction, &http_client, &startgg_token, interaction.channel_id).await?.into_iter().next() { //TODO handle multiple eligible races
        let mut team = None;
...
pub(crate) fn configure_builder(discord_builder: serenity_utils::Builder, db_pool: PgPool, http_client: reqwest::Client, config: Config, env: Environment, shutdown: rocket::Shutdown) -> serenity_utils::Builder {
    discord_builder
        ...
        .on_guild_create(false, |ctx, guild, _| Box::pin(async move {
            ...
            let assign = guild.create_application_command(ctx, |c| {
                c.name("assign");
                c.kind(serenity::model::application::command::CommandType::ChatInput);
                c.default_member_permissions(Permissions::ADMINISTRATOR);
                c.dm_permission(false);
                c.description("Marks this thread as the scheduling thread for the given start.gg set.");
                c.create_option(|o| o
                    .kind(CommandOptionType::String)
                    .name("startgg-set") //TODO Challonge support?
            ...
            let ban = if has_draft {
                Some(guild.create_application_command(ctx, |c| c
                    .name("ban")
            ...
            ctx.data.write().await
                .entry::<CommandIds>()
                .or_default()
                .insert(guild.id, CommandIds { assign, ban, draft, first, post_status, pronoun_roles, schedule, second, skip, status, watch_roles });
                        ...
                        if interaction.data.id == command_ids.assign {
                            ...
                            if let Some(event_row) = sqlx::query!(r#"SELECT series AS "series: Series", event FROM events WHERE discord_guild = $1 AND end_time IS NULL"#, i64::from(guild_id)).fetch_optional(&mut transaction).await? {
                                ...
                                let high_seed = match event.draft_kind() {
                                    DraftKind::MultiworldS3 => Some(match interaction.data.options[1].resolved.as_ref().expect("missing slash command option") {
                                ...
                                response_content.push(" to schedule as a regular race, or ping a tournament organizer to schedule as an async."); //TODO adjust message if asyncing is not allowed
                                ...
                                if team.is_some() || interaction.member.as_ref().expect("/schedule called outside of a guild").permissions.expect("permissions should be included in interaction response").administrator() {
                                    ...
                                    if let Some(start) = parse_timestamp(start) {
                                        if start < Utc::now() + Duration::minutes(30) {
                                            ...
                                            //TODO include previous time(s) in response if any
